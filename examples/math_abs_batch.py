...
# daal4py abs example for shared memory systems
...
def main(readcsv=read_csv, method='defaultDense'):
    ...
    algo = d4p.math_abs(method=method)
    data = readcsv(infile)
    result2 = algo.compute(data)

    # covariance result objects provide correlation, covariance and mean
    assert np.allclose(result1.value, result2.value)
    assert np.allclose(result1.value, np.absolute(data.toarray() if hasattr(data, 'toarray') else data))
