...
# daal4py reLU example for shared memory systems
...
def main(readcsv=read_csv, method='defaultDense'):
    ...
    assert np.allclose(result1.value, np.maximum(0, (data.toarray() if hasattr(data, 'toarray') else data)))
...
if __name__ == "__main__":
    res = main()
    print("ReLU result (first 5 rows):\n", res.value[:5])
