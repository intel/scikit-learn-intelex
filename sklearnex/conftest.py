...
def pytest_configure(config):
    config.addinivalue_line("markers","allow_sklearn_fallback: mark test to not check for sklearnex usage")
...
def pytest_runtest_call(item):
    ...
    if not item.get_closest_marker("allow_sklearn_fallback"):
        ...
        if "fallback to original Scikit-learn" in log_stream.getvalue():
            raise TypeError("test evaluated with sklearn via fallback")
    else:
        yield