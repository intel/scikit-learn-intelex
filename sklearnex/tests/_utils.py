...
def gen_dataset(estimator, queue=None, target_df=None, dtype=np.float64):
    ...
    if target_df == "pandas" and np.issubdtype(dtype, np.integer):
        # Pandas float to int not allowed
        X = _convert_to_dataframe(
            X.astype(dtype), sycl_queue=queue, target_df=target_df
        )
        y = _convert_to_dataframe(
            y.astype(dtype), sycl_queue=queue, target_df=target_df
        )
    else:            
