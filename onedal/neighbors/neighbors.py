...
def _parse_auto_method(estimator, method, n_samples, n_features):
...
def _validate_data(estimator, X, y=None, reset=True,
    ...
    if check_params.get('ensure_2d', True):
        _check_n_features(X, reset=reset)
...
def _onedal_fit(self, X, y, queue):
    ...
    train_alg = _backend.neighbors.classification.train(policy, params,
                            _backend.from_numpy(X),
                            _backend.from_numpy(y))

    return train_alg

def _onedal_predict(self, model, X, queue):
    ...
    result = _backend.neighbors.classification.infer(policy, params, model, _backend.from_numpy(X))
...
class NeighborsBase(metaclass=ABCMeta):
    ...
    def _fit(self, X, y, queue):
        ...
        if y is not None or requires_y:
            X, y = _validate_data(self, X, y, dtype=[np.float64, np.float32])
            ...
            X, _ = _validate_data(self, X, dtype=[np.float64, np.float32])
            y = self._validate_targets(y, X.dtype)
...
class KNeighborsClassifier(NeighborsBase, ClassifierMixin):
    ...
    def predict(self, X, queue=None):
        ...
        responses = _backend.to_numpy(prediction_result.responses)
...
class KNeighborsMixin:
    def _kneighbors(estimator, X=None, n_neighbors=None,
        ...
        distances = _backend.to_numpy(prediction_results.distances)
        indices = _backend.to_numpy(prediction_results.indices)
